...
def check_claim(c, root):
    ...
    for ev in c.get("evidence", []):
        ...
        if kind == "verbatim":
            ...
            # PRESENCE claim: the pattern must appear at least `min` times in the
            # source. Unlike count-max, this catches DRIFT-TO-ABSENT — e.g. a
            # version string the README asserts but Cargo.toml no longer carries.
            # (count-max greens a 0-match because 0 <= max; count-min is the dual.)
            n, matched = _count(ev["pattern"], ev["glob"], root)
            if not matched:
                fails.append(f'predicate matched NO files (measures nothing): glob {ev["glob"]}')
            elif n < ev["min"]:
                fails.append(
                    ...
                    f'[/{ev["pattern"]}/]  — the doc asserts it but the source no longer shows it'
                )
        elif kind == "no-new":
            n, matched = _count(ev["pattern"], ev["glob"], root)
            if not matched:
